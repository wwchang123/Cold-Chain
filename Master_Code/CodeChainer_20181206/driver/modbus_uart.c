...
void Modbus_Event(void)
{
	...
	{
		if(master.State_flag==0x31) //从机模式
		{
			if(modbus.RceBuf[0]==modbus.MyAdd)//确定数据包是不是发到本设备的
			{
			...
			else if(modbus.RceBuf[0]==0)//广播地址
			{
...
void Modbus_Fun3(void)
{
	...
	u16 i,j;
	u16 crc;
	RegAdd=modbus.RceBuf[2]*256+modbus.RceBuf[3];//得到寄存器首地址
	RegLen=modbus.RceBuf[4]*256+modbus.RceBuf[5];//得到寄存器的个数
	i=0;
	modbus.SendBuf[i++]=modbus.MyAdd;//本机地址
	...
	crc=CRC16_Modbus(modbus.SendBuf,i);//计算出校验码
...
void Modbus_Fun6(void)
{
	...
	u16 i,j;
	u16 crc;
	RegAdd=modbus.RceBuf[2]*256+modbus.RceBuf[3];//得到要修改寄存器首地址
	vol=modbus.RceBuf[4]*256+modbus.RceBuf[5];//修改后的值 
	Reg[RegAdd]=vol;
	//以下为回应主机
	i=0;
//	
