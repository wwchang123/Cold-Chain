...
//const char *string = "AT+QIOPEN=\"TCP\",\"47.100.0.27\",\"18888\"";//IP登录服务器,需要根据自己的IP和端口做修改,0x0d,0x0a
//const char *string = "AT+QIOPEN=\"TCP\",\"139.192.152.127\",\"12000\"";//IP登录服务器,需要根据自己的IP和端口做修改,0x0d,0x0a
...
const char *string = "AT+QIOPEN=\"TCP\",\"195.34.89.241\",\"7\"";
...
u8 Connect_Server(void)
{
	...
	if(0==GSM_send_cmd((char*)string,"CONNECT",7))
	{
		//CLR_Buf2();	
		CLR_Buf2(Uart2_Buf,Buf2_Max);
		return 1;
...
u8 Wait_CREG(void)
{
	...
	printf("GPRS开始连接服务器....\r\n");
	delay_ms(1500);
	if(1==Connect_Server()) //表示已经连接上公网
	{
		GSM_ON;//灯亮
	//	GSM_send_cmd("AT+QILOCIP","OK",2);//附着GPRS业务
		return 1;
	}
