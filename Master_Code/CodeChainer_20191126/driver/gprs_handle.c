...
void GPRSData_Init(void)
{
	...
	memset(GPRSGET.Sensor_Data,0,400);
	memset(GPRSGET.Batch_version,0,2);
	memset(GPRSGET.GetIPadress,0,32);
					...
					{ 
							...
							switch(CMD)
							{
								case 0xFFFF:  //收到交换机心跳包
									...
									GPIO_ResetBits(GPIOB,GPIO_Pin_7);//表示网络正常
									if(GPRSGET.NewIPOK==0x01)
										Timer.NewIP_count++;
								break;
								case 0x0083:   //收到交换机重启的命令
									Restart_data=0x09;//网络平台重启
									Rstart_Save();
									NVIC_SystemReset();
								break;
