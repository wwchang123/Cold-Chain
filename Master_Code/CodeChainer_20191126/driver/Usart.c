...
extern u8 Restart_data;//SIM卡未插重启
u8 zf_count=0;//用来记录IP地址的字符个数 个数一旦不符合，则不进行读写操作
#if 1
...
void USART1_IRQHandler(void)//
{
  ...
  {	
		...
		if(UART1_Receive_Size>=99)
			UART1_Receive_Size=99;
				...
				while(1)
				{
					if(1==Do_config())
					{
						...
						if(zf_count==3)
						{
							...
							printf("数据输入出错！\r\n\r\n");	
						Restart_data=0x07;//第一次开机手动IP地址配置完成重启
						Rstart_Save();
						NVIC_SystemReset();   //复位
						break;
					}
...
void Uart_Config(u8 flag)
{
	if(flag==0x01)
	{
		if(Config.flash_flag==0x00)//表示要重新配置序列号和IP地址、端口号
		{
			if(Config.Num_Flash[0]=='w' && Config.Num_Flash[1]=='w' && Config.Num_Flash[2]=='c')
			{
				IWDG_Feed();// 喂狗
		...
		{
			printf("先输入序列号（序列号必须为6位数）：\r\n");
			CLR_Buf1(Config.Num_Flash,0x06);
			while(1)
			{
        if(Config.NumOK_flag==0x01)
				{
			...
			while(1)
			{
				...
				while(1)
				{
					if(1==Do_config())
					{
						...
						Restart_data=0x08;//手动IP地址配置完成重启
						Rstart_Save();
						NVIC_SystemReset();   //复位
						break;
					}
...
//	else
//	{
//		if(Uart1_Buf[0]=='w' && Uart1_Buf[1]=='w' && Uart1_Buf[2]=='c')
//		{
//			IWDG_Feed();// 喂狗
//			CLI();
//			Config.flash_flag=0x01;
