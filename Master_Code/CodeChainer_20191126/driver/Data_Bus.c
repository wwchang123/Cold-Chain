...
#define Version_L 0x26;  //更新版本更新为0x26
...
extern u8 Restart_data;
extern u8 rssi_data[2];
/*******************************************************************************
* 函数名 : Sum_Check
* 描述   : 计算发送数组的校验位
* 输入   : 数组的长度
...
//			}	
			Send_Buff[16]=0x00;
			Send_Buff[17]=rssi_data[0];  //信号
			Send_Buff[18]=rssi_data[1];  //信号 由于采用透传模式，不好采集信号
			Send_Buff[19]=Version_H;
			Send_Buff[20]=Version_L;
...
//===============	重新初始化GPRS==================
	...
	switch(GPRS_Init((char*)IP_COM.Gprsdwon_IP))		//GPRS注册
	{
		...
		case 3:   
			printf("GPRS注册成功!\r\n"); 
		  delay_ms(50);
//			Send_GPRS_DATA1(0x0003);  //连接完成后，发送开机命令  没有必要发送此包
