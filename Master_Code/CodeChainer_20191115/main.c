...
void Rstart_Save(void)  //为了重启模式存储数据
{
	if(Restart_data != state.type) //判断重启方式是否一致，一致就不用重新进行写操作了
	{
		CLI();
		SPI_Flash_Erase_Sector(0x09) ;//把第二个扇区全部清掉
...
int main(void)
{
  ...
  GetSenior_num();
	...
	while (1)
	{
			...
			GPRS_Handle();
			if(Timer.NoHeart_count>=240)//三分钟内没有收到心跳包
			{
				GPIO_SetBits(GPIOB,GPIO_Pin_7);//网络灯灭
				if(Timer.NoHeart_count>=900 && IPCOMG_CMD == 0x00)//15分钟内没有收到心跳包
				{
