...
float get_temp(u8 ch)
{
    ...
    s16 iTem;
    u8 cBottom, cTop,i;
    AD_u=Get_Adc_Average(ch,6);
//    AD_r = AD_u/Magn*AI_ma*1000;
    AD_r = (float)((7*AD_u+10000)*2000/(200000-7*AD_u));
    if(AD_r<RTD_TAB_PT100[0])
        return 0;
    if(AD_r>RTD_TAB_PT100[210])
        return 0;   
  cBottom = 0; 
    cTop    = 210;  
 for (i=20; (cTop-cBottom)!=1; )        // 2分法查表。
    {
        if (AD_r < RTD_TAB_PT100[i])
        {
            cTop = i;
            i = (cTop + cBottom) / 2;
        }
        else if (AD_r > RTD_TAB_PT100[i])
        {
            cBottom = i;
            i = (cTop + cBottom) / 2;
        }
        ...
        {
            iTem = (u16)i * 5 - 200;// i * 5因为温度表每步为5度，-200是因为表从-200度开始建立的，如果从0度建表就不需要减                                      减200了。
            fTem = (float)iTem;
    ...
    fTem = ( ((AD_r - fLowRValue)*5) / (fHighRValue - fLowRValue) ) + iTem;     // 表格是以5度为一步的。
                                                                                                                        // 两点内插进行运算。
