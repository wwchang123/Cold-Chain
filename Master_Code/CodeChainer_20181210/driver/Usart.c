...
{
	  memset(p,0,buf2);
    UART1_Receive_Size = 0;              //接收字符串的起始存储位置
}
...
void Uart5_Init(uint32_t BaudRate)
{
	...
	USART_Init(UART5, &USART_InitStructure);// Enable USART5 Receive interrupts 使能串口接收中断

	USART_ITConfig(UART5, USART_IT_RXNE, ENABLE);
	USART_Cmd(UART5, ENABLE);// Enable the USART3 
