...
void drv_spi_init( void )
{
	...
	//SPI外设配置
	CLK_PeripheralClockConfig( CLK_Peripheral_SPI1,ENABLE );		//开SPI时钟
	SPI_DeInit(SPI1);		//SPI复位
	//SPI外设初始化
...
uint8_t drv_spi_read_write_byte( uint8_t TxByte )
{
	...
	while( RESET == SPI_GetFlagStatus(SPI1,SPI_FLAG_TXE ) )		//等待发送缓冲区空
	{
	...
	SPI_SendData(SPI1,TxByte );	//发送数据
	l_WaitTime = SPI_WAIT_TIMEOUT / 2;
	while( RESET == SPI_GetFlagStatus(SPI1,SPI_FLAG_RXNE ) )	//等待接收缓冲区非空
	{
		if( SPI_WAIT_TIMEOUT == ++l_WaitTime )
		{
			break;			//等待超时
